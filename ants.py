...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 3
    min_range, max_range = 0, None

    def nearest_bee(self, hive, min_range=0, max_range=None):
        ...
        #Create a list of all the places excluding the hive
        places = []
        current_place = self.place
        while current_place!=hive and current_place!=None:
            places.append(current_place) 
            current_place = current_place.entrance

        #If no max_place was specified, change max place to the length of the places so each place (up until the hive) is tested for bees
        if max_range==None:
            max_range = len(places)

        #for each place in [places], check to see if there are bees. If there is a bee, return the bee. Otherwise, return None.
        for i in range(min_range, max_range):
            x = random_or_none(places[i].bees)
            if x:
                return x
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0:
            ...
            self.place.bees = [b for b in bees if b.armor>0]
            self.place.remove_insect(self)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        x = random_or_none(self.place.bees)
        if self.digesting>0:
            self.digesting -= 1
        elif x:
            self.eat_bee(x)
    ...
    def add_insect(self, insect):
        ...
        if insect.is_watersafe==False:
            if insect.is_ant:
                ...
                self.bees[-1].reduce_armor(self.bees[-1].armor)
...
class QueenAnt(ScubaThrower):
    ...
    food_cost = 7
    implemented = True   # Change to True to view in the GUI
    imposter_checker = False

    def __init__(self, armor=1):
        Ant.__init__(self, armor)
        if QueenAnt.imposter_checker==False:
            QueenAnt.imposter_checker = True
            self.imposter = False
    ...
    def action(self, colony):
        ...
        #If the queen is an imposter, reduce its armor to zero
        if self.imposter:
            Insect.reduce_armor(self, self.armor)

        #If the queen is not an imposter, then do its designated action
        else:
            #The queen itself has the same action as a throwerant, in addition to doubling all the damage of the ants behind it
            ThrowerAnt.action(self, colony)

            #if the first call to action hasn't occurred yet
            if self.first_action==False:
                #set self.first_action to True so the places behind the queen are not unnecessarily generated multiple times. 
                self.first_action = True

                #generate all places behind the queen up until the exit
                current_place = self.place
                while current_place!=None:
                    self.places_behind.append(current_place) 
                    current_place = current_place.exit
            
            #If the first call to action has already occurred
            for place in self.places_behind[1:]:
                if place.ant and place.ant not in self.buffed_ants:
                    place.ant.damage *= 2
                    self.buffed_ants.append(place.ant)
                if place.ant and place.ant.is_container and place.ant.contained_ant!=None and place.ant.contained_ant not in self.buffed_ants:
